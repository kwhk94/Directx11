...
{
	if (QueryPerformanceFrequency((LARGE_INTEGER *)&m_PerformanceFrequency))	//성능 카운터 하드웨어 체크
	{
		m_bHardwareHasPerformanceCounter = TRUE;
		QueryPerformanceCounter((LARGE_INTEGER *)&m_nLastTime);				//퍼포먼스 카운터
		m_fTimeScale = 1.0f / m_PerformanceFrequency;						
	}
	else
	{
		m_bHardwareHasPerformanceCounter = FALSE;
		m_nLastTime = ::timeGetTime();								// 현재시간을 입력
		m_fTimeScale = 0.001f;
	}

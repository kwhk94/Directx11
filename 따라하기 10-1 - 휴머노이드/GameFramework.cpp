...
bool CGameFramework::CreateRenderTargetView()
{
	...
	if (FAILED(hResult = m_pDXGISwapChain->GetBuffer(0, __uuidof(ID3D11Texture2D), 
		(LPVOID *)&pd3dBackBuffer))) return(false);
	if (FAILED(hResult = m_pd3dDevice->CreateRenderTargetView(pd3dBackBuffer, 
		NULL, &m_pd3dRenderTargetView))) return(false);
...
bool CGameFramework::CreateDirect3DDisplay()
{
	...
	for (UINT i = 0; i < nDriverTypes; i++)
	{
		...
		if (SUCCEEDED(hResult = D3D11CreateDeviceAndSwapChain(NULL, nd3dDriverType, NULL, 
			...
			&dxgiSwapChainDesc, &m_pDXGISwapChain, &m_pd3dDevice, &nd3dFeatureLevel, 
...
void CGameFramework::BuildObjects()
{
	...
	//카메라 변환 행렬을 생성한다. 
	D3DXVECTOR3 d3dxvEyePosition = D3DXVECTOR3(0.0f, 200.0f, -145.0f);
